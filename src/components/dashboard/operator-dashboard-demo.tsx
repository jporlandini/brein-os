...
/**
 * Brain OS — Operator Dashboard (standalone demo / preview entry-point)
 * Self-contained: no server imports, no auth, no fetch.
 * All real logic lives in operator-dashboard.tsx (production).
 */
...
const MOCK_QUESTS: Quest[] = [
  {
    ...
    description: "Implementar nuevo sistema de grilla responsive con animaciones CSS. Incluye versión mobile y tablet.",
  ...
  },
  {
    ...
    description: "Creación de 12 creativos para campaña de awareness. Brief aprobado. Entrega en dos tandas de 6.",
  ...
  },
  {
    ...
    description: "Esperando credenciales de sandbox de Transbank. Bloqueada hasta que el cliente entregue accesos.",
  ...
  },
  {
    ...
    description: "Versionar el manual de marca con los nuevos colores aprobados en reunión del 5 de junio.",
  ...
  },
  {
    ...
    description: "Esperando material de cliente. En espera de grabaciones originales para iniciar edición.",
...
const STATUS_CFG: Record<QuestStatus, { label: string; color: string; dot: string }> = {
...
function GuardianBar() {
  const [value, setValue]   = useState("");
  const [reply, setReply]   = useState<string | null>(null);

  function handleSend() {
    if (!value.trim()) return;
    setReply(`Procesando: "${value.trim()}" — El Guardian Agent está en modo demo.`);
    setValue("");
    setTimeout(() => setReply(null), 5000);
  }
    ...
    <div className="rounded-xl border border-slate-700/40 bg-[#0d0d1a] p-4 space-y-3">
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 h-9 rounded-lg border border-slate-700/50 bg-slate-900/80 px-3 text-xs font-mono text-slate-200 placeholder:text-slate-700 outline-none focus:border-fuchsia-500/40 transition-colors"
      ...
      {reply && (
        <div className="flex items-start gap-2">
          ...
          <p className="text-[11px] font-mono text-green-400 leading-relaxed">{reply}</p>
        </div>
      )}
    </div>
  );
...
function LiveClock() {
  ...
  return <span className="text-[10px] font-mono text-slate-600 tabular-nums">{time}</span>;
}
      ...
      <header className="sticky top-0 z-40 border-b border-slate-800/50 bg-[#07070f]/92 backdrop-blur-md">
        <div className="max-w-5xl mx-auto px-5 h-14 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span className="hidden sm:block text-[9px] font-mono text-slate-600 border border-slate-800 px-1.5 py-0.5 rounded tracking-widest uppercase">
              Demo
            </span>
          </div>
          ...
          <div className="relative z-10 flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="text-xl md:text-2xl font-bold text-white leading-tight text-balance">
                Bienvenido, <span className="text-fuchsia-300">Operador</span>
              </h1>
          ...
          <div className="relative z-10 mt-5 grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            {stats.map(({ label, value, color }) => (
              <div key={label} className="text-center p-3 rounded-lg bg-slate-900/50 border border-slate-800/60">
                <p className={cn("text-2xl font-mono font-bold tabular-nums leading-none", color)}>
                  {value}
                </p>
